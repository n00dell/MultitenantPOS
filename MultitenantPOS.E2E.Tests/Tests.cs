...
namespace MultitenantPOS.Module.E2E.Tests;

public class MultitenantPOSTests : IDisposable {
    const string BlazorAppName = "MultitenantPOSBlazor";
    const string WinAppName = "MultitenantPOSWin";
    const string AppDBName = "MultitenantPOS";
    EasyTestFixtureContext FixtureContext { get; } = new EasyTestFixtureContext();

    public MultitenantPOSTests() {
        FixtureContext.RegisterApplications(
            new BlazorApplicationOptions(BlazorAppName, string.Format(@"{0}\..\..\..\..\MultitenantPOS.Blazor.Server", Environment.CurrentDirectory)),
            new WinApplicationOptions(WinAppName, string.Format(@"{0}\..\..\..\..\MultitenantPOS.Win\bin\EasyTest\net8.0-windows\MultitenantPOS.Win.exe", Environment.CurrentDirectory))
        );
        FixtureContext.RegisterDatabases(new DatabaseOptions(AppDBName, "MultitenantPOSEasyTest", server: @"(local)"));
    }
    public void Dispose() {
